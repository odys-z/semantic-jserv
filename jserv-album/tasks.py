...
def make(c, zip=f'jserv-portfolio-{version}.zip'):
    ...
    try:
        ...
        with zipfile.ZipFile(zip, 'w', zipfile.ZIP_DEFLATED) as zipf:
            ...
            for rk, rv in resources.items():
                if "*" in rv:  # Handle wildcard (e.g., web-dist/*)
                    ...
                    if count == 0:
                        err = True
                        print(f'No files found in {rv}.', file=sys.stderr)
                    ...
                    if os.path.exists(file):
                        ...
                        err = True
                        print(f"Warning: Resource '{rk}': '{file}' not found, skipping.", file=sys.stderr)
            ...
            for rk, files in vol_files.items():
                for file in files:
                    ...
                    if os.path.exists(file):
                        ...
                        err = True
                        print(f"Warning: volume file '{file}' not found, skipping.", file=sys.stderr)
        ...
        print(f"Created ZIP file{' successfully' if not err else ' with errors'}: {distzip}")
